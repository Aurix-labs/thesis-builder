...
- 4 列固定：Rubric / Target / R/R / Risk
- Rubric 列 v 格式：`<total>/100`，sub 格式：`<passed>/20 ✓`
...
| 4 | 所有 CSS class 来自 template_base.css | 模板/CSS 改动后跑 pytest scripts/tests/test_render_html.py |
...
v5 起 HTML 由 [`scripts/render_html.py`](../scripts/render_html.py) + [`templates/report.html.j2`](../templates/report.html.j2) 一次产出，模板/CSS 改动后跑 `pytest scripts/tests/test_render_html.py` 校验。

## Bear-case section（v3.1 新增）

Phase 2.6 sub-agent 产出 bear-case.md，append 到 report.md 后会被 Phase 3 渲染为：

```html
<section class="card" id="bear-case">
  <div class="section-eyebrow">// BEAR CASE · 对立面观点</div>
  <h2>如果多头错了，错在哪里</h2>
  <!-- bear-case.md 内容 -->
</section>
```

CSS：复用 .card；section-eyebrow 用 `--semantic-down` 红色而非 accent lavender，区分于 multi-bull 论调。

nav 锚点需增加 `<a href="#bear-case">空头</a>`，nav 锚点数从 13 增至 14。
